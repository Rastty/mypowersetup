...
After approval, run `npm run publish:expansion:check -- --market pt` (replace `pt` with `si` or `ro`) first. Publication must remain fail-closed if any native-review evidence is missing.
