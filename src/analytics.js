...
const COPY = {
  ...
  ro: { label: "Opțiuni de analiză", title: "Ne ajuți să îmbunătățim calculatorul?", text: "Cu acordul tău folosim Google Analytics pentru măsurarea anonimă a vizitelor și utilizării funcțiilor. Fără acord, analiza nu se încarcă.", accept: "Permite analiza", reject: "Refuză", details: "Mai multe despre confidențialitate", detailsUrl: "/ro/" },
  pt: { label: "Opções de análise", title: "Ajuda-nos a melhorar a calculadora?", text: "Com o teu consentimento usamos Google Analytics para medir anonimamente as visitas e a utilização das funcionalidades. Sem consentimento, a análise não é carregada.", accept: "Permitir análise", reject: "Recusar", details: "Mais sobre privacidade", detailsUrl: "/pt/" },
  sl: { label: "Nastavitve analitike", title: "Nam pomagaš izboljšati kalkulator?", text: "Z dovoljenjem uporabljamo Google Analytics za anonimno merjenje obiskov in uporabe funkcij. Brez dovoljenja se analitika ne naloži.", accept: "Dovoli analitiko", reject: "Zavrni", details: "Več o zasebnosti", detailsUrl: "/si/" },
};
...
function trackGuideCalculatorClick(event) {
  ...
  const destination = classifyGuideCalculatorLink(link.getAttribute("href"), { origin: window.location.origin }); if (!destination) return;
  track("guide_to_calculator_click", { ...destination, source_zone: classifyGuideClickZone({ inPrimaryCta: Boolean(link.closest(".cta")), inRelated: Boolean(link.closest(".related")), inHeader: Boolean(link.closest(".article-header")) }) });
}
...
function init() {
  ...
  document.addEventListener("click", (event) => { const trigger = event.target.closest?.("[data-analytics-settings]"); if (trigger) { event.preventDefault(); openSettings(); return; } trackGuideCalculatorClick(event); trackSharedCalculatorClick(event); });
