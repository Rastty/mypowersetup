...
const CATEGORY_RULES = [
  ["battery", /\b(baterie|akumulátor|lifepo4|lithium|agm)\b/i],
  ["solar_panel", /\b(solární panel|fotovoltaický panel|solar panel)\b/i],
  ["inverter", /\b(měnič|invertor|inverter)\b/i],
  ["controller", /\b(mppt|solární regulátor|regulátor nabíjení)\b/i]
];
...
export function normalizeProduct(raw, merchantKey) {
  ...
  const text = [raw.name, raw.category, raw.description].filter(Boolean).join(" ");
  ...
  return {
    ...
    categoryPath: cleanText(raw.category),
...
export function extractSpecs(text = "") {
  ...
  return {
    ...
    powerW: matchNumber(normalized, /(\d+(?:[.,]\d+)?)\s*(?:w|wp)\b/i),
