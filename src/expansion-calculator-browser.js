...
const labels = {
  ro: {
    ...
    recommendationBadges: { recommended: "Recomandarea noastră", budget: "Cel mai mic preț", reserve: "Rezervă mai mare", alternative: "Alternativă compatibilă" }, purchaseChecks: ["Potrivire tehnică", "Link exact", "Livrare verificată"], whyFit: "De ce se potrivește", fitChecks: "Verificări de potrivire", beforeBuy: "Înainte de cumpărare", viewAt: (merchant) => `Vezi prețul la ${merchant}`,
  ...
  pt: {
    ...
    recommendationBadges: { recommended: "A nossa recomendação", budget: "Preço mais baixo", reserve: "Maior margem", alternative: "Alternativa compatível" }, purchaseChecks: ["Compatibilidade técnica", "Ligação exata", "Entrega verificada"], whyFit: "Porque é compatível", fitChecks: "Verificações de compatibilidade", beforeBuy: "Antes de comprar", viewAt: (merchant) => `Ver preço na ${merchant}`,
  ...
  si: {
    ...
    recommendationBadges: { recommended: "Naše priporočilo", budget: "Najnižja cena", reserve: "Več rezerve", alternative: "Združljiva alternativa" }, purchaseChecks: ["Tehnična ustreznost", "Točna povezava", "Preverjena dostava"], whyFit: "Zakaj ustreza", fitChecks: "Preverjanje ujemanja", beforeBuy: "Pred nakupom", viewAt: (merchant) => `Preveri ceno pri ${merchant}`,
...
function renderProductSection(id, products, gap = "", routePriority = {}) { const components = products.filter((item) => item.category !== "power_station"); const portable = products.filter((item) => item.category === "power_station"); const preferPortable = routePriority.preferPortable === true && portable.length > 0; const componentRoute = components.length ? `<section class="expansion-purchase-route${preferPortable ? " is-secondary" : ""}" data-purchase-route="components" data-purchase-route-priority="${preferPortable ? "secondary" : "primary"}"><h5>${escapeHtml(labels.componentRoute)}</h5><p>${escapeHtml(labels.componentRouteIntro)}</p><div class="result-grid">${components.map((item) => renderExpansionProduct(item, preferPortable ? "secondary" : "primary")).join("")}</div></section>` : ""; const portableRoute = portable.length ? `<section class="expansion-purchase-route is-portable${preferPortable ? " is-priority" : ""}" data-purchase-route="portable" data-purchase-route-priority="${preferPortable ? "primary" : "secondary"}"><h5>${escapeHtml(labels.portableRoute)}</h5><p>${escapeHtml(labels.portableRouteIntro)}</p><div class="result-grid">${portable.map((item) => renderExpansionProduct(item, preferPortable ? "primary" : "secondary")).join("")}</div></section>` : gap; const priorityNote = preferPortable ? `<p class="purchase-route-priority-note" data-purchase-route-priority-note>${escapeHtml(labels.portablePriority(routePriority.coverage.missing.map(expansionCategoryLabel).join(", ")))}</p>` : ""; const routes = preferPortable ? `${portableRoute}${componentRoute}` : `${componentRoute}${portableRoute}`; return `<section class="result-products" aria-labelledby="${id}"><h4 id="${id}">${labels.products}</h4><p>${labels.productsIntro}</p>${priorityNote}${routes}<p><small>${labels.affiliate}</small></p></section>`; }
