...
import { expansionPurchaseRoutePriority } from "./expansion-purchase-route-priority.js";
import { bindAffiliateImpressionTracking, trackAffiliateClick, trackVisibleAffiliateImpressions } from "./affiliate-analytics.js?v=20260902-product-impressions1";
import { decorateExpansionRecommendations } from "./expansion-recommendation-roles.js";
import { groupExpansionProductsForDisclosure } from "./expansion-product-disclosure.js";
...
const labels = {
  ro: {
    ...
    recommendationBadges: { recommended: "Recomandarea noastră", budget: "Cel mai mic preț", reserve: "Rezervă mai mare", alternative: "Alternativă compatibilă" }, purchaseChecks: ["Potrivire tehnică", "Link exact", "Livrare verificată"], viewAt: (merchant) => `Vezi prețul la ${merchant}`, portableViewAt: (merchant) => `Vezi soluția completă la ${merchant}`, compareAlternatives: (count) => `Compară alternativele (${count})`,
    componentRoute: "Instalație din componente", componentRouteIntro: "Combină numai componentele necesare din categoriile de mai jos; acestea nu trebuie cumpărate împreună cu stația portabilă.", portableRoute: "Alternativă portabilă completă", portableRouteIntro: "O singură stație portabilă poate înlocui această cale din componente doar dacă limitele ei acoperă întregul profil.", portablePriority: (missing) => `Pentru acest calcul, calea din componente nu are încă o potrivire verificată completă pentru: ${missing}. Stația portabilă de mai jos acoperă profilul complet verificat și este afișată prima.`, loadingProducts: "Verificăm produsele compatibile pentru rezultatul tău…", noProducts: "Niciun produs verificat nu acoperă încă toate limitele calculate. Folosește ghidurile de mai sus pentru o instalație pe componente; nu micșora cerințele doar pentru a forța o recomandare.", noPortableFit: "Produsele de mai jos acoperă părți din instalația pe componente, dar nicio stație portabilă verificată nu acoperă întregul profil.",
  ...
  pt: {
    ...
    recommendationBadges: { recommended: "A nossa recomendação", budget: "Preço mais baixo", reserve: "Maior margem", alternative: "Alternativa compatível" }, purchaseChecks: ["Compatibilidade técnica", "Ligação exata", "Entrega verificada"], viewAt: (merchant) => `Ver preço na ${merchant}`, portableViewAt: (merchant) => `Ver solução completa na ${merchant}`, compareAlternatives: (count) => `Comparar alternativas (${count})`,
    componentRoute: "Instalação por componentes", componentRouteIntro: "Combina apenas os componentes necessários das categorias abaixo; não tens de os comprar juntamente com a estação portátil.", portableRoute: "Alternativa portátil completa", portableRouteIntro: "Uma única estação portátil só substitui a instalação por componentes quando os seus limites cobrem todo o perfil.", portablePriority: (missing) => `Para este cálculo, a instalação por componentes ainda não tem correspondência verificada completa para: ${missing}. A estação portátil abaixo cobre o perfil completo verificado e aparece primeiro.`, loadingProducts: "Estamos a verificar produtos compatíveis para o teu resultado…", noProducts: "Nenhum produto verificado cobre ainda todos os limites calculados. Usa os guias acima para uma instalação por componentes; não reduzas os requisitos apenas para forçar uma recomendação.", noPortableFit: "Os produtos abaixo cobrem partes da instalação por componentes, mas nenhuma estação de energia portátil verificada cobre todo o perfil.",
  ...
  si: {
    ...
    recommendationBadges: { recommended: "Naše priporočilo", budget: "Najnižja cena", reserve: "Več rezerve", alternative: "Združljiva alternativa" }, purchaseChecks: ["Tehnična ustreznost", "Točna povezava", "Preverjena dostava"], viewAt: (merchant) => `Preveri ceno pri ${merchant}`, portableViewAt: (merchant) => `Poglej celovito rešitev pri ${merchant}`, compareAlternatives: (count) => `Primerjaj alternative (${count})`,
    componentRoute: "Sistem iz posameznih komponent", componentRouteIntro: "Združi samo potrebne komponente iz spodnjih kategorij; ni jih treba kupiti skupaj s prenosno elektrarno.", portableRoute: "Celovita prenosna alternativa", portableRouteIntro: "Ena prenosna elektrarna nadomesti sistem iz komponent le, če njene omejitve pokrijejo celoten profil.", portablePriority: (missing) => `Pri tem izračunu pot iz posameznih komponent še nima popolnoma preverjenega ujemanja za: ${missing}. Spodnja prenosna elektrarna pokriva celoten preverjeni profil, zato je prikazana prva.`, loadingProducts: "Preverjamo združljive izdelke za tvoj rezultat…", noProducts: "Noben preverjen izdelek še ne pokriva vseh izračunanih omejitev. Uporabi zgornje vodnike za sistem iz posameznih komponent; zahtev ne zmanjšuj samo zato, da bi dobil priporočilo.", noPortableFit: "Spodnji izdelki pokrijejo dele sistema iz posameznih komponent, vendar nobena preverjena prenosna elektrarna ne pokrije celotnega profila.",
...
async function renderPortugalProducts(calculation) {
  ...
  try {
    ...
    const routePriority = trackProductCoverage(recommendations, calculation);
    ...
    target.innerHTML = renderProductSection("pt-products-title", products, portableGap, routePriority);
...
async function renderSloveniaProducts(calculation) {
  ...
  try {
    ...
    target.innerHTML = renderProductSection("si-products-title", products, portableGap, routePriority);
    trackRenderedProductChoices(target);
  } catch {
    trackProductCoverage({}, calculation);
...
async function renderRomaniaProducts(calculation) {
  ...
  try {
    ...
    if (!products.length) { target.innerHTML = renderNoVerifiedProducts(); trackRenderedProductChoices(target); return; }
    const portableGap = recommendations.power_station.length ? "" : `<p class="result-products-empty">${escapeHtml(labels.noPortableFit)}</p>`;
    target.innerHTML = renderProductSection("ro-products-title", products, portableGap, routePriority);
    trackRenderedProductChoices(target);
  } catch {
    trackProductCoverage({}, calculation);
...
function trackProductCoverage(recommendations, calculation) {
  const priority = expansionPurchaseRoutePriority(recommendations, calculation, locale);
  const { coverage } = priority;
  ...
  if (priority.preferPortable) {
    track("purchase_route_prioritized", {
      market: locale,
      purchase_route: "portable",
      missing_categories: coverage.missing.join(","),
      component_product_count: priority.componentProductCount,
    });
  }
  return priority;
}
...
function renderProductLoading() { return `<p class="result-products-loading" data-recommendation-loading>${escapeHtml(labels.loadingProducts)}</p>`; }
...
function renderProductSection(id, products, gap = "", routePriority = {}) {
  const components = products.filter((item) => item.category !== "power_station");
  const portable = products.filter((item) => item.category === "power_station");
  const preferPortable = routePriority.preferPortable === true && portable.length > 0;
  const componentPriority = preferPortable ? "secondary" : "primary";
  const portablePriority = preferPortable ? "primary" : "secondary";

  const componentRoute = components.length
    ? `<section class="expansion-purchase-route${preferPortable ? " is-secondary" : ""}" data-purchase-route="components" data-purchase-route-priority="${componentPriority}"><h5>${escapeHtml(labels.componentRoute)}</h5><p>${escapeHtml(labels.componentRouteIntro)}</p>${renderExpansionProductCollection(components, componentPriority)}</section>`
    : "";
  const portableRoute = portable.length
    ? `<section class="expansion-purchase-route is-portable${preferPortable ? " is-priority" : ""}" data-purchase-route="portable" data-purchase-route-priority="${portablePriority}"><h5>${escapeHtml(labels.portableRoute)}</h5><p>${escapeHtml(labels.portableRouteIntro)}</p>${renderExpansionProductCollection(portable, portablePriority)}</section>`
    : gap;
  const priorityNote = preferPortable
    ? `<p class="purchase-route-priority-note" data-purchase-route-priority-note>${escapeHtml(labels.portablePriority(routePriority.coverage.missing.map(expansionCategoryLabel).join(", ")))}</p>`
    : "";
  const routes = preferPortable ? `${portableRoute}${componentRoute}` : `${componentRoute}${portableRoute}`;
  return `<section class="result-products" aria-labelledby="${id}"><h4 id="${id}">${labels.products}</h4><p>${labels.productsIntro}</p>${priorityNote}${routes}<p><small>${labels.affiliate}</small></p></section>`;
}

function renderExpansionProductCollection(products, routePriority) {
  const groups = groupExpansionProductsForDisclosure(products);
  const primaryCards = groups
    .filter((group) => group.primary)
    .map((group) => renderExpansionProduct(group.primary, routePriority))
    .join("");
  const alternatives = groups
    .filter((group) => group.alternatives.length)
    .map((group) => renderExpansionAlternatives(group, routePriority))
    .join("");

  return `<div class="result-grid expansion-primary-products" data-primary-product-grid>${primaryCards}</div>${alternatives}`;
}

function renderExpansionAlternatives(group, routePriority) {
  const category = expansionCategoryLabel(group.category);
  return `<details class="expansion-product-alternatives" data-product-alternatives data-category="${escapeHtml(group.category)}"><summary><span>${escapeHtml(category)}</span><strong>${escapeHtml(labels.compareAlternatives(group.alternatives.length))}</strong></summary><div class="result-grid">${group.alternatives.map((item) => renderExpansionProduct(item, routePriority)).join("")}</div></details>`;
}
...
function renderExpansionProduct(item, routePriority = "secondary") { const solar = item.category === "solar_panel"; const portable = item.category === "power_station"; const fit = solar ? labels.solarFit(item.quantity, item.powerW) : portable ? labels.powerStationFit : labels.componentFit; const localeTag = { pt: "pt-PT", ro: "ro-RO", si: "sl-SI" }[locale]; const price = Number.isFinite(item.price) ? `<small class="expansion-product-price">${new Intl.NumberFormat(localeTag, { style: "currency", currency: item.currency || "EUR" }).format(item.price)}</small>` : ""; const category = ({ battery: labels.battery, solar_panel: labels.solar, controller: labels.mppt, inverter: labels.inverter, dc_charger: labels.dcCharger, shore_charger: labels.shoreCharger, power_station: labels.powerStation })[item.category] || item.category; const specs = expansionProductSpecs(item); const roles = item.recommendationBadges || ["alternative"]; const badges = roles.map((role) => `<span class="recommendation-role recommendation-role-${escapeHtml(role)}">${escapeHtml(labels.recommendationBadges[role])}</span>`).join(""); const merchant = expansionMerchantLabel(item.merchant); const checks = labels.purchaseChecks.map((check) => `<span>${escapeHtml(check)}</span>`).join(""); const attributedAffiliateUrl = decorateAwinAffiliateUrl(item.affiliateUrl, { market: locale, category: item.category, recommendationRole: roles[0], routePriority }); const primaryPortable = portable && routePriority === "primary"; const ctaLabel = primaryPortable ? labels.portableViewAt(merchant) : labels.viewAt(merchant); return `<article class="result-card expansion-product-card"><div class="recommendation-roles">${badges}</div><span>${escapeHtml(category)}</span><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(fit)}</small>${specs ? `<small>${escapeHtml(specs)}</small>` : ""}${price}<div class="expansion-purchase-checks" aria-label="${escapeHtml(labels.productsIntro)}">${checks}</div><a class="button button-primary" data-affiliate-product data-source="product-card" data-recommendation-role="${escapeHtml(roles[0])}" data-route-priority="${escapeHtml(routePriority)}" data-cta-variant="${primaryPortable ? "portable-primary" : "standard"}" data-product-id="${escapeHtml(item.id)}" data-category="${escapeHtml(item.category)}" data-merchant="${escapeHtml(item.merchant || (locale === "pt" ? "allpowers_pt" : "allpowers_eu"))}" href="${escapeHtml(attributedAffiliateUrl)}" rel="sponsored nofollow noopener" target="_blank">${escapeHtml(ctaLabel)}</a></article>`; }
function expansionCategoryLabel(category) { return ({ battery: labels.battery, solar_panel: labels.solar, controller: labels.mppt, inverter: labels.inverter, dc_charger: labels.dcCharger, shore_charger: labels.shoreCharger })[category] || category; }
function expansionMerchantLabel(merchant) { return ({ allpowers_pt: "ALLPOWERS", allpowers_eu: "ALLPOWERS", powerqueen_eu: "Power Queen", oxe_ro: "OXE", oxe_si: "OXE", ampul_eu: "AMPUL", xdatou: "Xdatou", bluetti_eu: "BLUETTI" })[merchant] || labels.viewProduct; }
