...
function renderProductSection(id, products, gap = "") { const components = products.filter((item) => item.category !== "power_station"); const portable = products.filter((item) => item.category === "power_station"); const componentRoute = components.length ? `<section class="expansion-purchase-route" data-purchase-route="components"><h5>${escapeHtml(labels.componentRoute)}</h5><p>${escapeHtml(labels.componentRouteIntro)}</p><div class="result-grid">${components.map(renderExpansionProduct).join("")}</div></section>` : ""; const portableRoute = portable.length ? `<section class="expansion-purchase-route is-portable" data-purchase-route="portable"><h5>${escapeHtml(labels.portableRoute)}</h5><p>${escapeHtml(labels.portableRouteIntro)}</p><div class="result-grid">${portable.map(renderExpansionProduct).join("")}</div></section>` : gap; return `<section class="result-products" aria-labelledby="${id}"><h4 id="${id}">${labels.products}</h4><p>${labels.productsIntro}</p>${componentRoute}${portableRoute}<p><small>${labels.affiliate}</small></p></section>`; }
