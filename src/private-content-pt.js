...
export function renderPortugalPrivateContentPage(pathname) {
  ...
  const topCta = page.type === "guide" ? `<p class="cta" data-guide-conversion-early-cta><a class="button button-primary" href="${calculatorHref}">Calcular e ver produtos compatíveis</a></p>` : "";
  const body = page.type === "hub"
    ...
    : `${(page.sections || []).map(([heading, text]) => `<section><h2>${escapeHtml(heading)}</h2><p>${escapeHtml(text)}</p></section>`).join("")}<aside class="cta"><h2 data-guide-conversion-cta>Calcula e vê produtos compatíveis</h2><p>Resultado gratuito e sem registo. No fim mostramos apenas produtos cujos limites técnicos e destino exato conseguimos verificar.</p><a class="button button-primary" href="${calculatorHref}">Abrir calculadora e ver produtos</a></aside>`;
