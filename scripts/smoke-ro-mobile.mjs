...
try {
  ...
  const result = await evaluate(cdp, `(() => { const links = [...document.querySelectorAll('[data-affiliate-product]')]; return { cards:links.length,href:links[0]?.href,rel:links[0]?.rel,merchant:links[0]?.dataset.merchant,invalidPanel:links.some((link) => /oxe-sp100w-panou-solar/i.test(decodeURIComponent(link.href))),scrollWidth:document.documentElement.scrollWidth,width:innerWidth }; })()`);
  assert(result.cards > 0, "no verified RO recommendation"); assert(result.invalidPanel === false, "OXE SP100W panel was recommended as a power station"); assert(result.href?.includes("awinmid=38934"), `wrong affiliate ${result.href}`); assert(result.href?.includes("ued="), "exact destination missing"); assert(result.rel?.includes("sponsored"), "rel sponsored missing"); assert(result.merchant === "allpowers_eu", `merchant=${result.merchant}`); assert(result.scrollWidth <= result.width + 2, `result overflow=${result.scrollWidth}`);
