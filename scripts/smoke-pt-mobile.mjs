...
try {
  ...
  assert(initial.lang === "pt", `lang=${initial.lang}`); assert(initial.width === 390, `width=${initial.width}`); assert(initial.scrollWidth <= 392, `overflow=${initial.scrollWidth}`); assert(/noindex/.test(initial.robots || ""), "noindex missing");
...
function findChrome(){ for(const candidate of [process.env.CHROME_BIN,"google-chrome","google-chrome-stable","chromium","chromium-browser"].filter(Boolean)){ if(spawnSync(candidate,["--version"],{encoding:"utf8"}).status===0)return candidate;} throw new Error("PT_MOBILE_SMOKE_CHROME_NOT_FOUND"); }
async function waitForHttp(url){ for(let i=0;i<60;i++){ try{const r=await fetch(url);if(r.ok)return;}catch{} await delay(100);} throw new Error("PT_MOBILE_SMOKE_PREVIEW_TIMEOUT"); }
async function waitForDevToolsPort(directory){ const path=`${directory}/DevToolsActivePort`; for(let i=0;i<120;i++){ try{const [p]=(await readFile(path,"utf8")).trim().split(/\r?\n/); if(Number(p)>0)return Number(p);}catch{} await delay(100);} throw new Error("PT_MOBILE_SMOKE_DEVTOOLS_TIMEOUT"); }
async function waitForChromeTarget(port){ for(let i=0;i<80;i++){ try{const r=await fetch(`http://127.0.0.1:${port}/json/list`); if(r.ok){const t=(await r.json()).find((x)=>x.type==="page"&&x.webSocketDebuggerUrl);if(t)return t;}}catch{} await delay(100);} throw new Error("PT_MOBILE_SMOKE_TARGET_TIMEOUT"); }
async function waitFor(predicate){ for(let i=0;i<100;i++){ if(await predicate())return; await delay(100);} throw new Error("PT_MOBILE_SMOKE_CONDITION_TIMEOUT"); }
async function evaluate(cdp,expression){ const r=await cdp.send("Runtime.evaluate",{expression,awaitPromise:true,returnByValue:true,userGesture:true}); if(r.exceptionDetails)throw new Error("PT_MOBILE_SMOKE_EVALUATION_FAILED"); return r.result?.value; }
function assert(condition,message){ if(!condition)throw new Error(`PT_MOBILE_SMOKE_ASSERTION:${message}`); }
