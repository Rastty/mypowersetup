...
test("Polish charging guides preserve the shared bounded charging model", async () => {
  const [hub, dcDc, shore] = await Promise.all([
    readFile("pl/poradnik/index.html", "utf8"),
    readFile("pl/poradnik/jak-dobrac-ladowarke-dc-dc/index.html", "utf8"),
    readFile("pl/poradnik/jak-dobrac-ladowarke-230-v/index.html", "utf8"),
  ]);
  assert.ok(hub.includes('href="/pl/poradnik/jak-dobrac-ladowarke-dc-dc/"'));
  assert.ok(hub.includes('href="/pl/poradnik/jak-dobrac-ladowarke-230-v/"'));
  for (const html of [dcDc, shore]) {
    for (const value of ["90%", "0,2 C", "0,1 C"]) assert.ok(html.includes(value));
    assert.ok(html.includes('href="/pl/#kalkulator"'));
  }
  for (const value of ["27,8 A", "30 A", "34 A", "67 A"]) assert.ok(dcDc.includes(value));
...
test("every market measures anonymous product coverage and ships a fresh calculator asset", async () => {
  ...
  for (const [htmlPath, appPath, locale] of markets) {
    ...
    assert.ok(html.includes("20260902-product-roles1"), `${htmlPath} nemá novou verzi aktiv`);
...
test("calculator assets are cache-busted and submit errors are visible", async () => {
  ...
  assert.ok(html.includes('src="/src/app.js?v=20260907-awin-clickref1"'));
...
test("language switch remains available on mobile", async () => {
  ...
  assert.ok(czech.includes('href="/styles.css?v=20260902-product-roles1"'));
  assert.ok(slovak.includes('href="/styles.css?v=20260902-product-roles1"'));
});
...
test("Slovak calculator is localized, indexable and isolated from Czech products", async () => {
  ...
  assert.ok(html.includes('src="/src/app-sk.js?v=20260907-awin-clickref1"'));
...
test("Polish calculator is localized, indexable and isolated to its verified catalog", async () => {
  ...
  assert.ok(html.includes('src="/src/app-pl.js?v=20260907-awin-clickref1"'));
...
test("all calculators offer a clean printable PDF summary", async () => {
  ...
  for (const html of [czech, slovak, polish]) {
    ...
    assert.ok(html.includes('/styles.css?v=20260902-product-roles1'));
  }
  for (const source of [app, appSk, appPl]) {
