...
test("Hungarian private page template stays noindex while committed HU output is public", async () => {
  ...
  assert.match(html, /app-hu-browser\.js\?v=20260902-visible-impressions1/);
...
test("Hungarian wiring guide preserves the bounded voltage-drop model", () => {
  const wiring = renderHungarianGuide("wiring");
  assert.match(wiring, /2 × egyirányú hossz/);
  assert.match(wiring, /1500 W, 12 V/);
  assert.match(wiring, /24,3 mm²/);
  assert.match(wiring, /25 mm² csak feszültségesés alapján/);
  assert.match(wiring, /megszakítóképesség/);
});

test("Hungarian fridge guide connects measured daily consumption to existing battery capacity", () => {
  const fridge = renderHungarianGuide("fridge");
  assert.match(fridge, /12 V × 5 A × 24 h × 0,40 = 576 Wh\/nap/);
  assert.match(fridge, /268 Wh\/24 h/);
